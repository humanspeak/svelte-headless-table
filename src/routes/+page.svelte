<script lang="ts">
    ...
    const columns = table.createColumns([
        ...
        table.group({
            header: (_, { rows, pageRows }) => derived([rows, pageRows], ([_rows, _pageRows]) => `Name (${_rows.length} records, ${_pageRows.length} in page)`),
            columns: [
                table.column({
                    header: (cell) => {
                        return createRender(
            ...
            columns: [
                ...
                table.column({
                    ...
                    plugins: {
                        ...
                        filter: {
                            fn: matchFilter,
                            render: ({ filterValue, preFilteredValues }) => createRender(SelectFilter, { filterValue, preFilteredValues })
                        },
                        tableFilter: {
                            exclude: true
                        },
                ...
                table.column({
                    ...
                    plugins: {
                        ...
                        filter: {
                            ...
                            render: ({ filterValue, values }) => createRender(NumberRangeFilter, { filterValue, values })
                        }
                    }
                }),
                table.column({
    ...
    const { headerRows, pageRows, tableAttrs, tableBodyAttrs, visibleColumns, pluginStates } = table.createViewModel(columns)
                ...
                <tr {...attrs}>
                    {#each headerRow.cells as cell (cell.id)}
                        <Subscribe attrs={cell.attrs()} let:attrs props={cell.props()} let:props>
                            <th {...attrs} on:click={props.sort.toggle} class:sorted={props.sort.order !== undefined} use:props.resize>
                                <div>
