...
export abstract class BodyRow<Item, Plugins extends AnyPlugins = AnyPlugins> extends TableComponent<Item, Plugins, 'tbody.tr'> {
    cells: BodyCell<Item, Plugins>[]
    /**
...
export class DataBodyRow<Item, Plugins extends AnyPlugins = AnyPlugins> extends BodyRow<Item, Plugins> {
    // TODO Workaround for https://github.com/vitejs/vite/issues/9528
    __data = true

    dataId: string
    original: Item
    constructor({ id, dataId, original, cells, cellForId, depth = 0, parentRow }: DataBodyRowInit<Item, Plugins>) {
        super({ id, cells, cellForId, depth, parentRow })
        this.dataId = dataId
        this.original = original
    }

    clone({ includeCells = false, includeSubRows = false }: BodyRowCloneProps = {}): DataBodyRow<Item, Plugins> {
...
export class DisplayBodyRow<Item, Plugins extends AnyPlugins = AnyPlugins> extends BodyRow<Item, Plugins> {
    ...
    clone({ includeCells = false, includeSubRows = false }: BodyRowCloneProps = {}): DisplayBodyRow<Item, Plugins> {
 ...
 */
export const getColumnedBodyRows = <Item, Plugins extends AnyPlugins = AnyPlugins>(rows: BodyRow<Item, Plugins>[], columnIdOrder: string[]): BodyRow<Item, Plugins>[] => {
 ...
 */
export const getSubRows = <Item, Plugins extends AnyPlugins = AnyPlugins>(subItems: Item[], parentRow: BodyRow<Item, Plugins>, { rowDataId }: BodyRowsOptions<Item> = {}): BodyRow<Item, Plugins>[] => {
        ...
        const cellForId = Object.fromEntries(
            Object.values(parentRow.cellForId).map((cell) => {
                ...
                if (column.isData()) {
                    ...
                    const value = dataCol.getValue(item)
                    return [column.id, new DataBodyCell({ row: subRows[rowIdx], column, label: column.cell, value })]
                }
                if (column.isDisplay()) {
                    return [column.id, new DisplayBodyCell({ row: subRows[rowIdx], column, label: column.cell })]
                }
                throw new Error('Unrecognized `FlatColumn` implementation')
            })
        )
