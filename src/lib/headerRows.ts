...
import { DataHeaderCell, FlatDisplayHeaderCell, GroupDisplayHeaderCell, GroupHeaderCell, type HeaderCell } from './headerCells.js'
...
export class HeaderRow<Item, Plugins extends AnyPlugins = AnyPlugins> extends TableComponent<Item, Plugins, 'thead.tr'> {
    cells: HeaderCell<Item, Plugins>[]
...
export const getHeaderRows = <Item, Plugins extends AnyPlugins = AnyPlugins>(columns: Column<Item, Plugins>[], flatColumnIds: string[] = []): HeaderRow<Item, Plugins>[] => {
...
export const getHeaderRowMatrix = <Item, Plugins extends AnyPlugins = AnyPlugins>(columns: Column<Item, Plugins>[]): Matrix<HeaderCell<Item, Plugins>> => {
    ...
    return rowMatrix.map((cells, rowIdx) =>
        cells.map((cell, columnIdx) => {
            ...
            if (rowIdx === maxHeight - 1) return new FlatDisplayHeaderCell({ id: columnIdx.toString(), colstart: columnIdx })
...
export const getOrderedColumnMatrix = <Item, Plugins extends AnyPlugins = AnyPlugins>(columnMatrix: Matrix<HeaderCell<Item, Plugins>>, flatColumnIds: string[]): Matrix<HeaderCell<Item, Plugins>> => {
...
export const headerRowsForRowMatrix = <Item, Plugins extends AnyPlugins = AnyPlugins>(rowMatrix: Matrix<HeaderCell<Item, Plugins>>): HeaderRow<Item, Plugins>[] => {
 ...
 */
export const getMergedRow = <Item, Plugins extends AnyPlugins = AnyPlugins>(cells: HeaderCell<Item, Plugins>[]): HeaderCell<Item, Plugins>[] => {
    if (cells.length === 0) {
        return cells
    }
