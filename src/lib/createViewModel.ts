...
import type { AnyPlugins, DeriveFlatColumnsFn, DeriveRowsFn, DeriveFn, PluginStates } from './types/TablePlugin.js'
...
export const createViewModel = <Item, Plugins extends AnyPlugins = AnyPlugins>(
    ...
    const pluginStates = Object.fromEntries(Object.entries(pluginInstances).map(([key, pluginInstance]) => [key, pluginInstance.pluginState])) as PluginStates<Plugins>
    ...
    const deriveTableHeadAttrsFns: DeriveFn<TableHeadAttributes<Item>>[] = Object.values(pluginInstances)
    ...
    const deriveTableBodyAttrsFns: DeriveFn<TableBodyAttributes<Item>>[] = Object.values(pluginInstances)
        .map((pluginInstance) => pluginInstance.deriveTableBodyAttrs)
        .filter(nonUndefined)
