...
export type DataColumnInit<Item, Plugins extends AnyPlugins = AnyPlugins, Id extends string = string, Value = unknown> = DataColumnInitBase<Item, Plugins, Value> &
...
export class GroupColumn<Item, Plugins extends AnyPlugins = AnyPlugins> extends Column<Item, Plugins> {
    // TODO Workaround for https://github.com/vitejs/vite/issues/9528
    __group = true
...
export const getFlatColumnIds = <Item, Plugins extends AnyPlugins = AnyPlugins>(columns: Column<Item, Plugins>[]): string[] => columns.flatMap((c) => (c.isFlat() ? [c.id] : c.isGroup() ? c.ids : []))

export const getFlatColumns = <Item, Plugins extends AnyPlugins = AnyPlugins>(columns: Column<Item, Plugins>[]): FlatColumn<Item, Plugins>[] => {
