...
export const createPageStore = ({ items, initialPageSize, initialPageIndex, serverSide, serverItemCount }: PageStoreConfig) => {
    ...
    function calcPageCountAndLimitIndex([$pageSize, $itemCount]: [$pageSize: number, $itemCount: number]) {
...
export const addPagination =
    <Item>({ initialPageIndex = 0, initialPageSize = 10, serverSide = false, serverItemCount }: PaginationConfig = {}): TablePlugin<
        Item,
        PaginationState,
        Record<string, never>,
