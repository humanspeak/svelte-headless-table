...
export const addGroupBy =
    <Item>({ initialGroupByIds = [], disableMultiGroup = false, isMultiGroupEvent = isShiftClick }: GroupByConfig = {}): TablePlugin<Item, GroupByState, GroupByColumnOptions<Item>, GroupByPropSet> =>
    ({ columnOptions }) => {
        ...
        return {
            ...
            hooks: {
                ...
                'tbody.tr.td': (cell) => {
                    const props: Readable<GroupByPropSet['tbody.tr.td']> = derived([repeatCellIds, aggregateCellIds, groupCellIds], ([$repeatCellIds, $aggregateCellIds, $groupCellIds]) => {
                        return {
