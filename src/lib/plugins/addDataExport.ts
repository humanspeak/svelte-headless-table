...
const getObjectsFromRows = <Item>(rows: BodyRow<Item>[], ids: string[], childrenKey: string): Record<string, unknown>[] => {
    return rows.map((row) => {
...
export const addDataExport =
    <Item, F extends DataExportFormat = 'object'>({ format = 'object' as F, childrenKey = 'children' }: DataExportConfig<F> = {}): TablePlugin<Item, DataExportState<F>, DataExportColumnOptions> =>
    ({ tableState, columnOptions }) => {
        ...
        const exportedData = derived([rows, exportedIds], ([$rows, $exportedIds]) => {
            switch (format) {
                case 'json':
                    return JSON.stringify(getObjectsFromRows($rows, $exportedIds, childrenKey)) as DataExport<F>
                case 'csv':
