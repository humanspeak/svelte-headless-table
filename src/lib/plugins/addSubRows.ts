...
const withSubRows = <Item, Row extends DataBodyRow<Item>>(row: Row, getChildren: ValidChildrenFn<Item>): Row => {
...
export const addSubRows =
    <Item>({ children }: SubRowsConfig<Item>): TablePlugin<Item, Record<string, never>, Record<string, never>, NewTablePropSet<never>> =>
    () => {
        const getChildren: ValidChildrenFn<Item> = children instanceof Function ? children : (item) => item[children] as unknown as Item[]
