...
const isAllSubRowsSelectedForRow = <Item>(row: BodyRow<Item>, $selectedDataIds: Record<string, boolean>, linkDataSubRows: boolean): boolean => {
    if (row.isData()) {
...
const isSomeSubRowsSelectedForRow = <Item>(row: BodyRow<Item>, $selectedDataIds: Record<string, boolean>, linkDataSubRows: boolean): boolean => {
    if (row.isData()) {
        if (!linkDataSubRows || row.subRows === undefined) {
            return $selectedDataIds[row.dataId] === true
        }
...
const writeSelectedDataIds = <Item>(row: BodyRow<Item>, value: boolean, $selectedDataIds: Record<string, boolean>, linkDataSubRows: boolean): void => {
    if (row.isData()) {
...
const getRowIsSelectedStore = <Item>(row: BodyRow<Item>, selectedDataIds: RecordSetStore<string>, linkDataSubRows: boolean): Writable<boolean> => {
...
export const addSelectedRows =
    <Item>({ initialSelectedDataIds = {}, linkDataSubRows = true }: SelectedRowsConfig<Item> = {}): TablePlugin<Item, SelectedRowsState<Item>, Record<string, never>, SelectedRowsPropSet> =>
    ({ tableState }) => {
