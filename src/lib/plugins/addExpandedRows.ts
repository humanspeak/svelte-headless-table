...
const withExpandedRows = <Item, Row extends BodyRow<Item>>(row: Row, expandedIds: Record<string, boolean>): Row[] => {
    ...
    const expandedSubRows = row.subRows.flatMap((subRow) => withExpandedRows<Item, Row>(subRow as Row, expandedIds))
    return [row, ...expandedSubRows]
}

export const addExpandedRows =
    <Item>({ initialExpandedIds = {} }: ExpandedRowsConfig<Item> = {}): TablePlugin<Item, ExpandedRowsState<Item>, Record<string, never>, NewTablePropSet<never>> =>
    () => {
