...
export const createSortKeysStore = (initKeys: SortKey[]): WritableSortKeys => {
    ...
    const toggleId = (id: string, { multiSort = true, toggleOrder = DEFAULT_TOGGLE_ORDER }: ToggleOptions = {}) => {
        update(($sortKeys) => {
            ...
            return [...$sortKeys.slice(0, keyIdx), { id, order: nextOrder }, ...$sortKeys.slice(keyIdx + 1)]
        })
    }
...
const getSortedRows = <Item, Row extends BodyRow<Item>>(rows: Row[], sortKeys: SortKey[], columnOptions: Record<string, SortByColumnOptions>): Row[] => {
...
export const addSortBy =
    <Item>({ initialSortKeys = [], disableMultiSort = false, isMultiSortEvent = isShiftClick, toggleOrder, serverSide = false }: SortByConfig = {}): TablePlugin<
        Item,
        SortByState<Item>,
        SortByColumnOptions,
        SortByPropSet
