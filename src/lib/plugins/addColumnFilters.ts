...
const getFilteredRows = <Item, Row extends BodyRow<Item>>(rows: Row[], filterValues: Record<string, unknown>, columnOptions: Record<string, ColumnFiltersColumnOptions<Item>>): Row[] => {
...
export const addColumnFilters =
    <Item>({ serverSide = false }: ColumnFiltersConfig = {}): TablePlugin<Item, ColumnFiltersState<Item>, ColumnFiltersColumnOptions<Item>, ColumnFiltersPropSet> =>
...
export const numberRangeFilter: ColumnFilterFn<[number | null, number | null], number> = ({ filterValue: [min, max], value }) => {
