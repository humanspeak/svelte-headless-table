...
function parseFrontmatter(src) {
    ...
    for (const line of body.split(/\r?\n/)) {
        ...
        if (!m) continue
        let value = m[2]
        // Strip surrounding quotes (frontmatter values may be wrapped in either
        // flavor — both string literals are required for the check to be
        // exhaustive). The second pair uses double-quoted single-quote literals,
        // which the project's singlequote rule flags; the alternative would be
        // escaping the quote inside a single-quoted string, which reads worse.
        if (
            (value.startsWith('"') && value.endsWith('"')) ||
            // trunk-ignore(eslint/quotes): testing for a literal single-quote char
            (value.startsWith("'") && value.endsWith("'"))
        ) {
